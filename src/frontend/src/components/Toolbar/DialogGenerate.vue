...
import { createLineageLink, deepClone, forEveryDragon, makeError } from "../../app/utils";
...
onMounted(async () => { 
    ...
    if(saveReqs.failed){
        status.value.error(`To save on the server, lineages must be between ${settings.gens.min}
