...
function validateGenderFitsBreed(dragon: DragonType) {
    ...
    // for this breed
        ...
        return dragon.gender === breed.genderOnly;
...
function verifyIntegrity(root: LineageRoot, supressReasoning = false){
    ...
    const tests: Array<((dragon: DragonType) => boolean)> = [
        function breed(dragon) { return validateBreed(dragon.breed) },
        // Some of these tests will also fail if the above breed test fails
        function name(dragon) { return validateName(dragon.name) },
        function code(dragon) { return validateCode(dragon.code) },
        function display(dragon) { return validateDisplay(dragon.display) },
        function gender(dragon) { return validateGender(dragon.gender) },
        function genderFitsBreed(dragon) { return validateGenderFitsBreed(dragon) }
    ];
