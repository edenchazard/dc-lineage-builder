...
await mkdir('./assets/tile-rendering/', { recursive: true });
...
try {
  ...
  await writeFile('./utils/shared/breed-definitions.ts', await response.text());
  ...
  const sprites = [
    ...
    const filePath = `./assets/tile-rendering/${filename}`;
